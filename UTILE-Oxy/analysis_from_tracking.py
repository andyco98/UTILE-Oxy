...
def plot_new_bubbles_vs_time(csv_path, voltage):
    """
    ...
    # Customize the plot
    plt.title(f"Temporal Evolution of New Bubbles for {voltage}", fontsize=28)
...
def plot_merging_events_vs_time(csv_path, voltage):
    """
    ...
    # Customize the plot
    plt.title(f"Temporal Evolution of Merge Events for {voltage}", fontsize=28)
    plt.xlabel("Frame Number", fontsize=18)
...
def generate_heatmap(csv_path, voltage):
    """
    ...
    plt.title(f"Bubble Emerging Points Density Map for {voltage}", fontsize=28)
    ...
    heatmap_save_path = os.path.join(analysis_save_path, f"Bubble Emerging Points Heatmap for {voltage}.png")
...
def generate_merging_heatmap(csv_path, voltage):
    """
    ...
    # Set title with the desired font size
    plt.title(f"Bubble Merging Points Density Map for {voltage}", fontsize=28)
    ...
    mrg_heatmap_save_path = os.path.join(analysis_save_path, f"Bubble Merging Points Density Map for {voltage}.png")
...
def analyze_bubble_trajectories_and_velocities(csv_path, voltage):
    """
    ...
    Args:
        ...
        voltage (str): Voltage associated with the dataset (e.g., "1500mV").
    ...
    save_path = Path(csv_path).parent / f"Analysis Plots for {voltage}"
    ...
    plt.title(f"Bubble Trajectories for {voltage}", fontsize=28)
    ...
    # Save the plot
    plot_save_path = save_path / f"trajectory_{voltage}_with_metrics.png"
...
def bubble_metrics_analysis(csv_path, movement_threshold, voltage):
    """
    ...
    Args:
        ...
        voltage (str): Voltage level associated with the dataset.
    ...
    # Define save path
    parent_directory = Path(csv_path).parent
    analysis_save_path = parent_directory / f"Analysis Plots for {voltage}"
    analysis_save_path.mkdir(parents=True, exist_ok=True)
    ...
    summary_csv_path = analysis_save_path / f"bubble_metrics_summary_{voltage}.csv"
